...
This is technically not a well-defined definition of a limit, because what is meant by "really close" and "approaches"? There is a more technical version of the definition of a limit written out in the text.

Roughly speaking, it defines two small numbers, $\delta$ and $\epsilon$. For a limit to exist, if $f(x)$ is within $\epsilon$ units of the limit in question, then there needs to exist a value for $\delta$ that is greater than the difference between $x$ and $c$.