...
This is particularly helpful in proving some more difficult limits, like $\lim_{x\to0}\frac{\sin x}{x}=1$. A simpler one to prove is $\lim_{x\to\infty}\frac{\sin x}{x}=0$, and to do that we'll use the fact that $\sin x$ falls between $-1$ and $1$.
