...
Finding the derivative $\frac{dy}{dx}$ means finding out the rate of change of $y$ with respect to $x$, or to put it another way, how does $y$ change as $x$ change?
...
Something to keep in mind is that you $\ln$ is undefined for negative numbers, so you might run into $\ln|u|$ with the absolute value there as a safeguard. You can pretend the absolute value is not there when differentiating as the results are the same.
...
$$\frac{d}{dx}[a^x] = e^{(\ln a)x} \cdot \ln a = a^x \cdot \ln a $$

And we can do something similar to logarithms with other bases. Our base change rule is

$$ \log_a b = \frac{\log_c b}{\log_c a} $$

where $c$ is whatever new base we want. Well, in our case we want the base to be $e$ since we know that rule already.
