...
For various reasons, the typical way differential equations are constructed is in differential form, rather than what we saw above. So, the more common version of the problem above is $dy=2\,dx$. From there we can apply our new operation of **antidifferentiation**, which uses the integral symbol $\int$.

$$\begin{align}
\int dy = \int 2 \, dx
\end{align}$$

The one immediate benefit to using the differential setup is that we can immediately see the **variable of integration**. So on the right side, we know whatever function we come up with should have an $x$ as its variable.
...
So far, each solution we've found for a differential equation has been the general solution, $2x + C$. A **particular solution** means that you also know some initial condition that will allow you to determine the constant.
