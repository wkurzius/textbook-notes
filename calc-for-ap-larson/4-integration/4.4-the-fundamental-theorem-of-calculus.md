...
Let's create a function, $A(x)$, that will produce the area under a curve, but $x$ will be the right bound of the interval. These are typically called **accumulation functions** since the area accumulates as $x$ increases.
...
Recall that in this situation, $f(x)$ was the curve and $A(x)$ is the area under that curve. So, if the derivative of an integral is its function (equation $\ref{eq:1}$), that means the integral of a function is equal to the antiderivative of that function (equation $\ref{eq:2}$) and is *commonly* known as the first part of the fundamental theorem and can be a couple of different ways.

$$\begin{align}
F(x) &= \int_a^x f(t) \, dt \label{eq:fun-1a}
\end{align}$$
...
This is the Mean Value Theorem for Integrals, which we can apply to finding the average value of a function on an interval.
